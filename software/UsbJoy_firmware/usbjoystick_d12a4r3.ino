		...
		if ((ch>='0') && (ch<='9')) {
			...
			switch (uart_command) {
				...
				default:
					...
					Serial.println("'lX#'=set smart-led X (volatile) #={off,on,100ms,on pressed,flashing,sin(),flashing pressed}");
					Serial.println("'mX#'=set smart-led X to EEPROM #={0=off,1=on,100ms,on pressed,flashing,sin(),flashing pressed}");
